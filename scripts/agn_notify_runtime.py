...
LEGACY_OUTBOX_PATH = RUNTIME_DIR / "kirara_outbox.jsonl"
...
def _migrate_legacy_outbox(target: Path) -> None:
    if target.exists() or not LEGACY_OUTBOX_PATH.exists():
        return
    target.parent.mkdir(parents=True, exist_ok=True)
    LEGACY_OUTBOX_PATH.replace(target)
...
def enqueue_message(
    *,
    ...
    _migrate_legacy_outbox(outbox_path)

    payload = {
        "message_id": f"agnmsg-{uuid4().hex[:16]}",
    ...
    if append_memory_event is not None:
        try:
            append_memory_event(
                key=f"agn_telegram_outbox:{payload['message_id']}",
                payload=payload,
                kind="agn_telegram_outbox_message",
                source=str(source or "agn"),

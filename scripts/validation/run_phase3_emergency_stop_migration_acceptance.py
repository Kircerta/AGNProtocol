...
def _migration_checks() -> dict[str, Any]:
    script_payload_cmd = (
        "import json, emergency_stop as es; "
        "print(json.dumps({'package_path': es.PACKAGE_PATH, 'legacy_script_shim': es.LEGACY_SCRIPT_SHIM}, ensure_ascii=True))"
    )
    script_show = _run(
    ...
    package_show = _run(
        [
            ...
            "-c",
            (
                "from agn.core.emergency_stop import PACKAGE_PATH, LEGACY_SCRIPT_SHIM; "
    ...
    shim_text = (ROOT / "scripts" / "emergency_stop.py").read_text(encoding="utf-8")
    ...
    return {
        "script_surface": {
            ...
            "pass": int(script_show.returncode) == 0 and str(script_payload.get("package_path", "")) == "agn.core.emergency_stop",
        },
        "package_surface": {
            "returncode": int(package_show.returncode),
            "package_path": str(package_payload.get("package_path", "")),
            "pass": int(package_show.returncode) == 0 and str(package_payload.get("package_path", "")) == "agn.core.emergency_stop",
        },
        "script_shim_static": {
            "shim_import_present": "from agn.core.emergency_stop import *" in shim_text,
            "pass": "from agn.core.emergency_stop import *" in shim_text,
        },
        "system_import": {
            "package_import_present": "from agn.core.emergency_stop import initialize_system_mode, load_system_mode" in system_text,
            "pass": "from agn.core.emergency_stop import initialize_system_mode, load_system_mode" in system_text,
        },
        "read_models_import": {
            "package_import_present": "from agn.core.emergency_stop import load_system_mode" in read_models_text,
            "pass": "from agn.core.emergency_stop import load_system_mode" in read_models_text,
        },
        "control_daemon_import": {
            "package_import_present": "from agn.core.emergency_stop import activate_emergency_stop, is_emergency_stop_active, release_emergency_stop" in control_daemon_text,
            "pass": "from agn.core.emergency_stop import activate_emergency_stop, is_emergency_stop_active, release_emergency_stop" in control_daemon_text,
        },
    }
...
def build_report() -> dict[str, Any]:
    ...
    return {
        "schema_version": "agn.validation.phase3_emergency_stop_migration.v1",
        "generated_at": utc_now_iso(),
        "objective": "Verify that emergency-stop logic now lives in src/agn/core while the legacy script remains a compatible shim and active governance modules import the package implementation.",
        "binary_verdict": {
...
def main() -> int:
    ...
    report_path = REPORT_DIR / f"{timestamp}-phase3-emergency-stop-migration-acceptance.json"
    latest = REPORT_DIR / "phase3-emergency-stop-migration-acceptance.latest.json"

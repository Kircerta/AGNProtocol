...
from agn.governance import lifecycle
...
def test_package_lifecycle_exposes_metadata() -> None:
    assert lifecycle.PACKAGE_PATH == "agn.governance.lifecycle"
    assert lifecycle.LEGACY_SCRIPT_SHIM == "scripts/lifecycle_governance.py"


def test_package_integrity_sweep_detects_missing_artifact() -> None:
    ...
    summary = lifecycle.integrity_sweep()
...
def test_package_lifecycle_index_contains_delivered_runs() -> None:
    ...
    out = lifecycle.rebuild_index()
    index_path = Path(__file__).resolve().parents[1] / str(out["index"])
